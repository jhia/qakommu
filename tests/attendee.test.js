...
describe('Attendees API', () => {
  ...
  describe('Get Attendees', () => {
    it('should show all available attendees', async () => {
      const res = await request(app).get('/api/attendee').set(headers)
    ...
    it('should show one attendees', async () => {
      const res = await request(app).get('/api/attendee/3').set(headers)
      expect(res.statusCode).toEqual(200)
      expect(res.body.successful).toEqual(true)
      expect(res.body).toHaveProperty('payload')
    ...
    it('should show error if the id is not valid', async () => {
      const res = await request(app).get('/api/attendee/jenkins').set(headers)
